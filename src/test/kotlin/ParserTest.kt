...
class ParserTest {
    ...
    fun testTry(){
        for(p in parsers){
            ...
            Assert.assertEquals(p.fromString("try if true then 1 else 2 with 5"),
                TmTryWith(TmIf(TmBool(true), TmNum(1), TmNum(2)), TmNum(5))
            )
    ...
    fun opTest(){
        for(p in parsers)
        {
            ...
            Assert.assertEquals(p.fromString("if true then 1 else 2 + 4"),
                TmIf(TmBool(true), TmNum(1), AddOp(TmNum(2), TmNum(4)))
            )
            Assert.assertEquals(p.fromString("(if true then 1 else 2) + 4"),
                AddOp(TmIf(TmBool(true), TmNum(1), TmNum(2)), TmNum(4))
            )
