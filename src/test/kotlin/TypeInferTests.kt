...
class TypeInferTests {
    ...
    fun applySubsTests() {
        ...
        Assert.assertEquals("int list->bool list", applySubs(
            mutableListOf("X0=int", "X1=int", "X2=bool"),
            "X0 list->X2 list"
        )
        )
    ...
    fun typeInferTests(implicit : Boolean) {
        ...
        Assert.assertEquals("int", typeInfer(
            TmIf(TmBool(true), TmNum(1), TmNum(3)),
            identTable(hashMapOf()), implicit
        )
        )

        // Expressão = if true then true else false
        Assert.assertEquals("bool", typeInfer(
            TmIf(TmBool(true), TmBool(true), TmBool(false)),
            identTable(hashMapOf()), implicit
        )
        )
        ...
        Assert.assertEquals("bool", typeInfer(
            TmIf(EqOp(TmNum(3), TmNum(2)), TmBool(true), TmBool(false)),
            identTable(hashMapOf()), implicit
        )
        )
        ...
        // Expressão = 3 - 2
        Assert.assertEquals("int", typeInfer(
            SubOp(TmNum(3), TmNum(2)), identTable(hashMapOf()),
            implicit
        )
        )
        ...
        Assert.assertEquals("int", typeInfer(
            TmHd(TmList(Vnum(3), TmList(Vnum(4), TmNil()))),
            identTable(hashMapOf()), implicit
        )
        )
        ...
        Assert.assertEquals("int list", typeInfer(
            TmTl(TmList(Vnum(3), TmList(Vnum(4), TmNil()))),
            identTable(hashMapOf()), implicit
        )
        )
        ...
        Assert.assertEquals("int list", typeInfer(
            TmTl(TmList(Vnum(3), TmNil())),
            identTable(hashMapOf()), implicit
        )
        )

        // Expressão = cons(false, nil)
        Assert.assertEquals("bool list", typeInfer(
            TmList(Vbool(false), TmNil()),
            identTable(hashMapOf()), implicit
        )
        )
        ...
        Assert.assertEquals("(int->bool)", typeInfer(
            TmFn(TmVar("x"), TyInt(), EqOp(TmVar("x"), TmNum(3))),
            identTable(hashMapOf()), implicit
        )
        )

        // Expressão = fn x:int => x + 3
        Assert.assertEquals("(int->int)", typeInfer(
            TmFn(TmVar("x"), TyInt(), AddOp(TmVar("x"), TmNum(3))),
            identTable(hashMapOf()), implicit
        )
        )
        ...
        Assert.assertEquals("bool", typeInfer(
            TmApp(TmFn(TmVar("x"), TyInt(), EqOp(TmVar("x"), TmNum(3))), TmNum(2)),
            identTable(hashMapOf()), implicit
        )
        )
        ...
        // Expressão = (fn x:int => x + 3) 2
        Assert.assertEquals("int", typeInfer(
            TmApp(TmFn(TmVar("x"), TyInt(), AddOp(TmVar("x"), TmNum(3))), TmNum(2)),
            identTable(hashMapOf()), implicit
        )
        )
        ...
        Assert.assertEquals("int", typeInfer(
            TmLet(TmVar("x"), TyInt(), TmNum(3), AddOp(TmVar("x"), TmNum(2))),
            identTable(hashMapOf()), implicit
        )
        )
        ...
        // Expressão = let x:int = 3 in x == 2
        Assert.assertEquals("bool", typeInfer(
            TmLet(TmVar("x"), TyInt(), TmNum(3), EqOp(TmVar("x"), TmNum(2))),
            identTable(hashMapOf()), implicit
        )
        )
        ...
        Assert.assertEquals("int", typeInfer(
            TmLet(TmVar("x"), TyInt(), SubOp(TmNum(4), TmNum(1)), AddOp(TmVar("x"), TmNum(2))),
            identTable(hashMapOf()), implicit
        )
        )
        ...
        Assert.assertEquals("int", typeInfer(
            TmLet(
                TmVar("x"), TyBool(), EqOp(TmNum(3), TmNum(2)),
        ...
        //              in sum(0)
        val sum_k = TmLetRec(
            TmVar("sum"), TyInt(), TyInt(), TmVar("k"),
            TmIf(
                GrOp(TmVar("k"), TmNum(7)),
        ...
        Assert.assertEquals("int", typeInfer(
            TmTryWith(AddOp(TmNum(2), TmNum(1)), TmRaise()),
            identTable(hashMapOf()), implicit
        )
        )
        ...
        //              with raise
        Assert.assertEquals("int", typeInfer(
            TmTryWith(sum_k, TmRaise()),
            identTable(hashMapOf()), implicit
        )
        )

        Assert.assertEquals("bool list", typeInfer(
            TmCat(TmBool(true), TmNil()),
            identTable(hashMapOf()), implicit
        )
        )

        Assert.assertEquals("int list", typeInfer(
            TmCat(TmNum(1), TmCat(TmNum(2), TmNil())),
            identTable(hashMapOf()), implicit
        )
        )
    ...
    fun fnErrorTest2() {
        typeInfer(
            TmFn(TmVar("x"), TyInt(), AddOp(TmVar("x"), TmBool(false))),
    ...
    fun letErrorTest() {
        typeInfer(
            TmLet(TmVar("x"), TyInt(), TmNum(3), AddOp(TmVar("x"), TmBool(false))),
            identTable(hashMapOf()), true
        )
    }
